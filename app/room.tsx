...
export default function MultiplayerRoomScreen() {
  ...
  const leaveQueueMutation = trpc.matchmaking.leave.useMutation({
    onSuccess: () => {
      void queueStatusQuery.refetch();
    },
  });
  ...
  useEffect(() => {
    if (queueStatusQuery.data?.status === "matched" && queueStatusQuery.data.matchedRoomId) {
      setRoomId(queueStatusQuery.data.matchedRoomId);
    }
  }, [queueStatusQuery.data]);
  const room = roomQuery.data;
  const displayedRoom = realtimeRoom ?? room;
  const currentQuestion = room && room.status === "playing" ? questions[room.currentQuestionIndex % questions.length] : null;
      ...
      <ScreenContainer className="px-5" containerClassName="bg-background">
        <View style={styles.center}>
          <View style={[styles.heroIcon, { backgroundColor: colors.primary }]}>
            <IconSymbol name="person.2.fill" size={28} color={colors.background} />
          </View>
          <Text style={[styles.title, { color: colors.foreground }]}>Sign in to play live.</Text>
          <Text style={[styles.subtitle, { color: colors.muted }]}>
            Private rooms use your account so scores, timeouts, and rewards stay secure.
          </Text>
          <Pressable
          ...
          >
            <Text style={[styles.primaryText, { color: colors.background }]}>Play as Demo Guest</Text>
          </Pressable>
          <Pressable
          ...
          >
            <Text style={[styles.primaryText, { color: colors.foreground }]}>Sign in with OAuth</Text>
          </Pressable>
        </View>
      </ScreenContainer>
    );

  if (!displayedRoom) return <ScreenContainer className="px-5" containerClassName="bg-background"><ScrollView contentContainerStyle={styles.content}>
    <Pressable onPress={() => router.back()}><Text style={[styles.back, { color: colors.primary }]}>‹ Back to play</Text></Pressable>
    <Text style={[styles.eyebrow, { color: colors.primary }]}>LIVE MULTIPLAYER</Text><Text style={[styles.title, { color: colors.foreground }]}>Private rooms. Same questions.</Text><Text style={[styles.subtitle, { color: colors.muted }]}>Create a room and share the six-digit code, or join a friend's room.</Text>
    {isQueued ? (
      <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.primary, borderWidth: 2 }]}>
        <View style={[styles.heroIcon, { backgroundColor: "#1C3C38" }]}>
          <IconSymbol name="bolt.fill" size={24} color={colors.success} />
        </View>
        <Text style={[styles.cardTitle, { color: colors.foreground }]}>Finding Ranked Match…</Text>
        <Text style={[styles.cardBody, { color: colors.muted }]}>
          Searching for an opponent in division: {queueStatusQuery.data?.division ?? "Bronze"}
        </Text>
        <Text style={[styles.reward, { color: colors.primary }]}>
          In Queue: {Math.floor(queueElapsed / 60)}:{String(queueElapsed % 60).padStart(2, "0")}
        </Text>
        <Pressable
        ...
        >
          <Text style={[styles.primaryText, { color: colors.foreground }]}>
            {leaveQueueMutation.isPending ? "Cancelling…" : "Cancel Search"}
          </Text>
        </Pressable>
      </View>
    ) : (
      <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.primary }]}>
        <View style={[styles.heroIcon, { backgroundColor: "#243650" }]}>
          <IconSymbol name="bolt.fill" size={22} color={colors.primary} />
        </View>
        <Text style={[styles.cardTitle, { color: colors.foreground }]}>Ranked Matchmaking</Text>
        <Text style={[styles.cardBody, { color: colors.muted }]}>
          Auto-match with players in your division for competitive season points and division ranking.
        </Text>
        <Pressable
          disabled={busy || joinQueueMutation.isPending}
        ...
        >
          <Text style={[styles.primaryText, { color: colors.background }]}>
            {joinQueueMutation.isPending ? "Entering Queue…" : "Find Ranked Match"}
          </Text>
        </Pressable>
      </View>
    )}
    <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={[styles.heroIcon, { backgroundColor: "#243650" }]}><IconSymbol name="person.2.fill" size={22} color={colors.primary} /></View><Text style={[styles.cardTitle, { color: colors.foreground }]}>Create a room</Text><Text style={[styles.cardBody, { color: colors.muted }]}>Each round gets a server-issued token and deadline.</Text><Pressable disabled={busy} onPress={() => createMutation.mutate({ mode: "bible_quiz" })} style={({ pressed }) => [styles.primaryButton, { backgroundColor: colors.primary }, pressed && styles.pressed]}><Text style={[styles.primaryText, { color: colors.background }]}>{createMutation.isPending ? "Creating…" : "Create private room"}</Text></Pressable></View>
    <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}><Text style={[styles.cardTitle, { color: colors.foreground }]}>Join a room</Text><TextInput value={roomCode} onChangeText={setRoomCode} keyboardType="number-pad" maxLength={6} placeholder="Enter 6-digit room code" placeholderTextColor={colors.muted} style={[styles.input, { borderColor: colors.border, color: colors.foreground }]} /><Pressable disabled={busy || roomCode.length !== 6} onPress={() => joinMutation.mutate({ roomCode })} style={({ pressed }) => [styles.primaryButton, { backgroundColor: roomCode.length === 6 ? colors.primary : colors.border }, pressed && styles.pressed]}><Text style={[styles.primaryText, { color: roomCode.length === 6 ? colors.background : colors.muted }]}>{joinMutation.isPending ? "Joining…" : "Join room"}</Text></Pressable></View>
    {(createMutation.error || joinMutation.error) && <Text style={[styles.error, { color: colors.error }]}>{createMutation.error?.message ?? joinMutation.error?.message}</Text>}
  ...
  return <ScreenContainer className="px-5" containerClassName="bg-background"><ScrollView contentContainerStyle={styles.content}>
    <View style={styles.topRow}><Text style={[styles.eyebrow, { color: colors.primary }]}>ROOM {roomState.roomCode}</Text><Text style={[styles.status, { color: roomState.status === "complete" ? colors.success : colors.muted }]}>{roomState.status}</Text></View>
    <Text style={[styles.connection, { color: socketColor }]}>{socketLabel}</Text>
